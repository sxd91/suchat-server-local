...
app.get('/api/v1/uploads', { preHandler: auth }, async (request) => ({ items: all('SELECT id,file_name AS fileName,mime_type AS mimeType,byte_size AS byteSize,storage_key AS storageKey,created_at AS createdAt FROM uploads WHERE user_id=? ORDER BY created_at DESC LIMIT 100', request.user.sub) }));
app.post('/api/v1/uploads/metadata', { preHandler: auth }, async (request, reply) => { const b = request.body || {}; if (requireBody(reply, b.fileName && b.mimeType, 'fileName_mimeType_required')) return; const id = makeId('upl'); const t = now(); const storageKey = b.storageKey || id + '-' + path.basename(b.fileName); run('INSERT INTO uploads (id,user_id,file_name,mime_type,byte_size,storage_key,created_at) VALUES (?,?,?,?,?,?,?)', id, request.user.sub, b.fileName, b.mimeType, Number(b.byteSize || 0), storageKey, t); return reply.code(201).send({ id, fileName: b.fileName, mimeType: b.mimeType, byteSize: Number(b.byteSize || 0), storageKey, createdAt: t }); });
